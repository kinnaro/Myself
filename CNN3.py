...
# theano.config.comput_test_value = 'warn'

'''
TRAIN_DIR = 'S:/InputTrain/'
TEST_DIR = 'S:/InputTest/'
'''
TRAIN_DIR_N = 'S:/INPUT/随机N/'
TRAIN_DIR_V = 'S:/INPUT4000/V3500/'

TEST_DIR = 'S:/INPUT4000/Test/'

# 把N和V分开读入
# 想全部读入的话，就把那个if去掉。
# 记得把label也存好（0/1）
train_V = [(TRAIN_DIR_V+i, 1) for i in os.listdir(TRAIN_DIR_V) if 'V' in i]
train_N = [(TRAIN_DIR_N+i, 0) for i in os.listdir(TRAIN_DIR_N) if 'N' in i]
